...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    rolls , sum, one_rolled = 0 , 0, False
...
def free_bacon(score):
    ...
    assert score >= 0, 'The score cannot be negative in the game.'
    # BEGIN PROBLEM 2
    ones_digit = score % 10   
    tens_digit = score // 10
    
    return 2 + abs(tens_digit - ones_digit)
    # END PROBLEM 2
...
def is_swap(score0, score1):
    ...
    assert score0 >= 0, "scores must be positive. score0 is not"
    assert score1 >= 0, "scores must be positive. score1 is not"
    # BEGIN PROBLEM 4
...
def commentary( largestDiff = 0 , prevSwap = None , prevLeader = None):
    """
    print comments on each turn and return function for future comments changing depending on current settings
    domain: scores of player0 , player1 , if a swap occurred

    this is the implementation for problem 6 in phase 2 of HOG
    """
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while( score0 < goal and score1 < goal ):
        ...
        # figure out number of dice by using strategy
        if( player == 0):
        ...
        if( is_swap( score0  , score1 ) ):
              ...
              score1 = temp
        #switch player
        player = other(player)  #(player + 1) % 2 # should use other function but this is more fun
...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def say( score0 , score1  ):
        """
        print highest change in score if new record set

        algorithm idea: compare score and print, hold onto other_high and other_low and toggle announce highest

        @return: announce_highest
        """
        #fence post difining first other_high and other_score 
        if( score0 == 0 or score1 == 0):
            other_previous_high , other_previous_score = 0 , 0

        #figure out which is current score
        if( who == 0 ):
            cur_score = score0
        else:
            cur_score = score1

        cur_diff = cur_score - previous_score
